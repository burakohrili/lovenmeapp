...
class ProfileSetupStep6Page extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<ProfileSetupStep6Page> createState() => _ProfileSetupStep6PageState();
}
...
class _ProfileSetupStep6PageState extends ConsumerState<ProfileSetupStep6Page> {
  ...
  void _sendRealSMS() async {
    ...
    if (phoneExists) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Bu telefon numarası ($fullPhoneNumber) zaten kayıtlı. Lütfen farklı bir numara deneyin.'),
  ...
  // Firebase'de telefon numarası kontrolü
  ...
  Future<void> _sendNetGsmSMS(String phoneNumber) async {
    try {
      ...
      final success = await NetGsmSmsService.sendOtpSms(
        phoneNumber,
        customMessage: 'Lovenme doğrulama kodunuz: {CODE}\n\nBu kodu kimseyle paylaşmayın.',
      );
  ...
  void _verifyRealSMS() async {
    ...
    try {
      // NetGSM OTP doğrulaması
      final phoneNumber = '$selectedCountryCode${_phoneController.text}';
      final isValid = NetGsmSmsService.verifyOtpCode(phoneNumber, enteredCode);
      ...
      if (user != null) {
        ...
        await FirebaseFirestore.instance
          ...
          .update({
            ...
            'phoneVerificationMethod': 'netgsm', // netgsm ile doğrulandığını belirt
          });
                ...
                if (!isCodeSent) ...[
                  ...
                  TextFormField(
                    ...
                    decoration: InputDecoration(
                      ...
                      hintText: selectedCountryCode == '+90' ? '5XX XXX XX XX' : 'Phone number',
                      ...
                      counterStyle: TextStyle(color: AppColors.white.withOpacity(0.5)),
                      filled: true,
